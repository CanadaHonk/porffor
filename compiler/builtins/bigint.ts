...
export const __Porffor_bigint_add = (a: f64, b: f64, sub: boolean): bigint => {
  ...
  const aNegative: boolean = Porffor.IR.loadU8(a, 0) != 0;
  const aLen: i32 = Porffor.IR.loadU16(a, 2);

  let bNegative: boolean = Porffor.IR.loadU8(b, 0) != 0;
  if (sub) bNegative = !bNegative;
  const bLen: i32 = Porffor.IR.loadU16(b, 2);

  const maxLen: i32 = Math.max(aLen, bLen);
  const digits: i32[] = Porffor.array.new(maxLen + 1);
  ...
  if (aNegative == bNegative) {
    ...
    for (let i: i32 = 0; i < maxLen; i++) {
      ...
      if (aOffset > 0) aDigit = Porffor.IR.loadI32(a + aOffset * 4, 0);

      let bDigit: i32 = 0;
      const bOffset: i32 = bLen - i;
      if (bOffset > 0) bDigit = Porffor.IR.loadI32(b + bOffset * 4, 0);
