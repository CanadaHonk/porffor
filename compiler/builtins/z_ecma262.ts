...
// These are runtime-generated functions for well-known symbols
// They are not available during precompilation, but will be generated by builtins.js
declare function __Symbol_toPrimitive(): symbol;
...
export const __ecma262_ToPrimitive_Number = (input: any): any => {
  ...
  // We access the well-known symbol via a declared function that will be generated at runtime
  // This avoids circular dependency (can't use input[Symbol.toPrimitive] as that calls ToPropertyKey)
  if (Porffor.comptime.flag`hasType.symbol`) {
    const toPrimSymbol: symbol = __Symbol_toPrimitive();
    if (Porffor.wasm`local.get ${toPrimSymbol}` != 0) {
      const exoticToPrim: any = __Porffor_object_get(input, toPrimSymbol);
      if (exoticToPrim != null && Porffor.type(exoticToPrim) == Porffor.TYPES.function) {
        // Call the exotic toPrimitive with hint "number" and this=input
        const result: any = Porffor.call(exoticToPrim, ['number'], input, null);
        if (result != null && !Porffor.object.isObjectOrNull(result)) return result;
...
export const __ecma262_ToPrimitive_String = (input: any): any => {
  ...
  // Check for Symbol.toPrimitive method (same approach as ToPrimitive_Number)
  if (Porffor.comptime.flag`hasType.symbol`) {
    const toPrimSymbol: symbol = __Symbol_toPrimitive();
    if (Porffor.wasm`local.get ${toPrimSymbol}` != 0) {
      const exoticToPrim: any = __Porffor_object_get(input, toPrimSymbol);
      if (exoticToPrim != null && Porffor.type(exoticToPrim) == Porffor.TYPES.function) {
        // Call the exotic toPrimitive with hint "string" and this=input
        const result: any = Porffor.call(exoticToPrim, ['string'], input, null);
        if (result != null && !Porffor.object.isObjectOrNull(result)) return result;
        throw new TypeError('Cannot convert an object to primitive');
      }
    }
