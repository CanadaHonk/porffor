    ...
    for (let _ = 0; _ < f.wasm.length; _++) {
      ...
      switch (i[0]) {
        ...
        case Opcodes.global_get: {
          const g = Object.values(globals).find(x => x.idx === i[1]);
          const name = invGlobals[i[1]];
          if (!g || !CValtype[g.type]) {
            vals.push(`${name}`);
            break;
          }
          ...
          const id = localTmpId++;
          const line2 = out.indexOf('{\n') + 2;
          out = out.slice(0, line2) + `  ${CValtype[g.type]} _get${id};\n` + out.slice(line2);
          line(`_get${id} = ${name}`);
          vals.push(`_get${id}`);
          break;
        }
...
// _memory = calloc(1, _memoryPages * ${PageSize});`);
  ...
  const makeIncludes = includes => [...includes.keys()]
    .filter(x => !['stdlib.h', 'string.h', 'time.h', 'math.h', 'stdio.h'].includes(x))
    .map(x => `#include <${x}>\n`).join('');
