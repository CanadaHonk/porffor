...
const stringifyOp = ({ types, funcs, codes, imports, globals }, op, porfFunc = op.porfFunc, depth = 0) => {
  ...
  if (porfFunc) {
    ...
    if (op.func != null) {
      str += ` \x1b[90m${op.func >= imports.length ? `${noHighlight(codes[op.func - imports.length].bc.porfFunc.name)}` : `${({ p: 'print', c: 'printChar', t: 'time', z: 'profile' })[imports[op.func].name]}`}`;
